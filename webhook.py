...
def create_erp_subscription(customer_name, stripe_sub_id, status):
    payload = {
        ...
        "subscription_plan": "Basic Test Subscription"
    }
    return requests.post(
...
def create_erp_invoice(customer_name, amount, stripe_invoice_id):
    ...
    print("Invoice payload:", payload, flush=True)
    return requests.post(
        f"{ERP_BASE_URL}/api/resource/Sales Invoice",
        headers=erp_headers,
        json=payload
...
def create_erp_subscription(customer_name, stripe_sub_id, status):
    ...
    print("Subscription payload:", payload, flush=True)
    return requests.post(
        f"{ERP_BASE_URL}/api/resource/Subscription",
        headers=erp_headers,
        json=payload
    ...
    if event_type == 'invoice.paid':
        ...
        if email:
            ...
            if erp_customer:
                ...
                print("Failed to get/create ERP customer.", flush=True)
        else:
            print("[invoice.paid] No email found — skipping ERPNext sync.", flush=True)

    elif event_type == 'customer.created':
        print("[customer.created] Ignored to avoid duplicate creation", flush=True)
        ...
        if email:
            ...
            if erp_customer:
                ...
                print("Failed to get/create ERP customer for subscription", flush=True)
